import { getOrCreateCart, syncCartWithUser, updateCartItem } from '@/lib/actions/cart-actions';
    ...
    persist(
        (set, get) => ({
            ...
            addItem: async (item) => {
                ...
                try {
                    // Check if item already exists in the cart by both id and sanityProductId
                    // We need to check both because the same product might have different cart item IDs
                    ...
                    // Also check if we have the same product with a different cart item ID
                    // This can happen if the product was added in a different session
                    const existingProductItem = items.find((i) => 
                        i.title === item.title && i.price === item.price
                    );

                    // If we found the same product with a different ID, update its quantity
                    if (existingProductItem) {
                        return get().updateQuantity(existingProductItem.id, existingProductItem.quantity + item.quantity);
                    }
